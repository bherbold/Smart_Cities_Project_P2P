...
best_case_scenarios =  [72] #[12, 245, 417, 124, 28, 418, 199, 417, 361, 417, 243] # 1 (change also line 386) or [12, 245, 417, 124, 28, 418, 199, 417, 361, 417, 243] from previously ran simulations
...
for load_profile_index2 in best_case_scenarios: # should be 656 or best_case_scenarios array
    ...
    for load_profile_index1 in range (656): # should be 656
        ...
        with open('summer_prosumers_no_EV.csv', newline='') as f1: # files winter_prosumers_no_EV.csv or summer_prosumers_no_EV.csv - change next if's accordingly
            reader = csv.reader(f1)
            for row in reader:
                if row[0] == "summer_pro1_no_EV":
                    continue
                else:
                    balance_HH3.append(float(row[0]))
                
                if row[1] == "summer_pro2_no_EV":
                    continue
                else:
                    balance_HH6.append(float(row[1]))
                
                if row[2] == "summer_pro3_no_EV":
                    continue
                else:
                    balance_HH8.append(float(row[2]))
                
                if row[3] == "summer_pro4_no_EV":
                    continue
                else:
                    balance_HH10.append(float(row[3]))

        f1.close()

        with open('winter_summer_consumers_no_EV.csv', newline='') as f_: # file winter_summer_consumers_no_EV.csv
            reader = csv.reader(f_)
            for row in reader:
                if row[1] == "Winter2jan":
                    continue
                else:
                    balance_HH1.append(float(row[1]))
                
                if row[2] == "Winter3jan":
                    continue
                else:
                    balance_HH5.append(float(row[2]))
                
                if row[3] == "Winter4jan":
                    continue
                else:
                    balance_HH7.append(float(row[3]))
                
                if row[4] == "Winter5jan":
                    continue
                else:
                    balance_HH9.append(float(row[4]))

        f_.close()

        balance_HH2 = [] # Consumer 2 with EV
        ...
        balance_HH4 = [] # Prosumer 2 with EV
